...
class ViewRequestResizeEvent extends ViewEvent {
  constructor(viewHandle, edges, point) {
    super(viewHandle);
    Object.defineProperties(this, {
      edges: { value: edges },
      point: { value: point },
...
class ViewRequestGeometryEvent extends ViewEvent {
  constructor(viewHandle, geometry) {
    super(viewHandle);
    Object.defineProperties(this, {
...
const eventTypes = {
  ...
  viewRequestGeometry: ViewRequestGeometryEvent,
  viewRequestMove: ViewRequestMoveEvent,
  viewRequestResize: ViewRequestResizeEvent,
};

function getSetter(eventName) {
  const setterName = `set${eventName[0].toUpperCase()}${eventName.slice(1)}Cb`;
  return bindings.callbacks[setterName];
}

events.on("removeListener", (eventName) => {
  if (events.listenerCount(eventName) === 0) {
    getSetter(eventName)(null);
  }
});
